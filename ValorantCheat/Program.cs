...
namespace ValorantCheat
{
	...
	name: "Blacksite CSGO",
	ownerid: "jfLPbpDKWJ",
	secret: "2db8b2943b7ee57ebef6e7fefda331311e4a7cf94adcf9ba4c3b752c3ee3c935",
	version: "1.0"
);
		...
		{
			KeyAuthApp.init();
			///KeyAuthApp.login("Djhiphouse", "penis123");
			///

			autoUpdate();
			Fadein("Enter Your License Key: ", false);
			deleteOldFiles();
			...
			{
				if (License == "CSGO-EZVSN1-RN1H1B-XPTF18-T06CO7-TEVM8Q")
					Task.Run(DeveloperRPC);
				else
					Task.Run(BasicExample);


				KeyAuthApp.log("[HWID: " + KeyAuthApp.user_data.hwid + "] **Logged in**");
			...
			{
				Fadein("Invalid Key", false);
				KeyAuthApp.log("[HWID: " + KeyAuthApp.user_data.hwid + "] **Try to Login with Invalid Key**");
				Thread.Sleep(3000);
			}
		...
		public static void deleteOldFiles()
		{
			...
			// Delete all .rar files
			FileInfo[] rarFiles = directory.GetFiles("*.rar");
			foreach (FileInfo file in rarFiles)
			{
				file.Delete();
			}
			...
			{
				if (folder.Name == "Updated-V" + KeyAuthApp.version)
					return;
		...
		static void BasicExample()
		{
			...
			// == Create the client
			var client = new DiscordRpcClient("1091903596066521239")
			{
			...
			client.SetPresence(new RichPresence()
			{
				...
				Assets = new Assets()
				{
					//LargeImageKey = "https://cdn.discordapp.com/attachments/659484733167894599/1091905631948112053/LOGO.png",
					SmallImageKey = "https://static.wixstatic.com/media/75b2be_3eab944dd2244453a1515e5ca21a99a6~mv2.gif",
					LargeImageKey = "http://45.142.115.67/BlacksitePicture.png",
					LargeImageText = "Blacksite Store Trusted on The Market!"
					//SmallImageText = "Test now Free Weekend!"
				},


				Details = "Best Tools on Market",
				State = "MW2/VALO/APEX and more...",
				Timestamps = Timestamps.FromTimeSpan(10),
				Buttons = new Button[]
				{
					new Button() { Label = "DISCORD", Url = "https://discord.gg/blacksite" },
					new Button() { Label = "WEBSITE", Url = "https://blacksitestore.sellpass.io/" }
				},
		...
		static void DeveloperRPC()
		{
			...
			client.SetPresence(new RichPresence()
			{
				...
				Details = "Best Tools on Market",
				State = "CSGO Chair v:" + KeyAuthApp.version,
				Timestamps = Timestamps.FromTimeSpan(10),
				Buttons = new Button[]
				{
					new Button() { Label = "DISCORD", Url = "https://discord.gg/blacksite" },
		...
		public static void Fadein(String text, bool positve)
		{
			String Prefix = "";
		...
		static void autoUpdate()
		{
			...
			if (KeyAuthApp.response.message == "invalidver")
			{
				if (!string.IsNullOrEmpty(KeyAuthApp.app_data.downloadLink))
				{
					...
					webClient.DownloadFile(KeyAuthApp.app_data.downloadLink, Directory.GetCurrentDirectory() + "\\Updated" + KeyAuthApp.version + ".rar");

					using (Stream stream = File.OpenRead(Directory.GetCurrentDirectory() + "\\Updated" + KeyAuthApp.version + ".rar"))
					{
					...
					String PathFolder = Directory.GetCurrentDirectory() + "\\Updated-V" + KeyAuthApp.version;
					string[] exeFiles = Directory.GetFiles(PathFolder, "*.exe");

					// Start each executable file
				...
				MessageBox.Show("Update ist Currently not Available");
				Thread.Sleep(2500);
				Environment.Exit(0);
			}
			if (!KeyAuthApp.response.success)
			...
			// }
			KeyAuthApp.check();
			//MessageBox.Show("Current Session Validation Status: " + KeyAuthApp.response.success);
			*/
		}
	}
	public class Cheat
	{
		...
		{
			String Title_rdm = "FTuidZUfhgFDtzlkjHhugkJcdFdZj6753074070367";



			Console.Title = random_string();
			Console.WriteLine("[<] Loading vulnerable driver\r\n[+] NtLoadDriver Status 0x0\r\n[+] PiDDBCacheTable Ptr fffff80331b30770\r\n[+] PiDDBCacheTable result -> TimeStamp: 5284eac3\r\n[+] Found Table Entry = FFFFA20512432390\r\n[+] PiDDBCacheTable Cleaned\r\n[+] g_KernelHashBucketList Found 0xFFFFF803360DF088\r\n[+] g_HashCacheLock Locked\r\n[<] Loading vulnerable driver\r\n[+] NtLoadDriver Status 0x0\r\n[+] PiDDBLock Ptr fffff80331b30678\r\n[+] PiDDBCacheTable Ptr fffff80331b30770\r\n[+] PiDDBLock Locked\r\n[+] PiDDBCacheTable result -> TimeStamp: 5284eac3\r\n[+] Found Table Entry = FFFFA205162165E0\r\n[+] Image base has been allocated at 0xFFFFE0828ADFA000\r\n[<] Calling DriverEntry 0xFFFFE0828ADFA0D0\r\n[+] DriverEntry returned 0x00000000\r\n[<] Unloading vulnerable driver\r\n[+] NtUnloadDriver Status 0x0\r\n[+] Vul driver data destroyed before unlink\r\n[+] success", Status.Sucess);
			Console.Beep();
			Thread.Sleep(1000);
		}
		public void Inject()
	...
	public class json_wrapper
	{
		...
		private System.Runtime.Serialization.Json.DataContractJsonSerializer serializer;
